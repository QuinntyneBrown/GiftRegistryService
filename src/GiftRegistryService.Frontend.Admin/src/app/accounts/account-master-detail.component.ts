...
import { GiftRegistryService } from "./account.service";
...
export class AccountMasterDetailComponent extends HTMLElement {
    constructor(
        private _GiftRegistryService: GiftRegistryService = GiftRegistryService.Instance	
	) {
        super();
    ...
    private async _bind() {
        this.accounts = await this._GiftRegistryService.get();
    ...
    public async onAccountAdd(e) {
        ...
        await this._GiftRegistryService.add(e.detail.account);
        this.accounts = await this._GiftRegistryService.get();
    ...
    public async onAccountDelete(e) {        
        await this._GiftRegistryService.remove({ id: e.detail.account.id });
        this.accounts = await this._GiftRegistryService.get();
